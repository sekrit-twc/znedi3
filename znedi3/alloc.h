...
#ifndef ZNEDI3_ALLOC_H_
#define ZNEDI3_ALLOC_H_
...
#ifdef _WIN32
  #include <malloc.h>
  inline void *znedi3_x_aligned_malloc(size_t size, size_t alignment) { return _aligned_malloc(size, alignment); }
  inline void znedi3_x_aligned_free(void *ptr) { _aligned_free(ptr); }
#else
  #include <stdlib.h>
  inline void *znedi3_x_aligned_malloc(size_t size, size_t alignment) { void *p; if (posix_memalign(&p, alignment, size)) return nullptr; else return p; }
  inline void znedi3_x_aligned_free(void *ptr) { free(ptr); }
#endif
...
struct AlignedAllocator {
	...
	{
		T *ptr = static_cast<T *>(znedi3_x_aligned_malloc(n * sizeof(T), ALIGNMENT));
